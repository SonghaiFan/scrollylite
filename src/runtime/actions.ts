...
export function hasScrollAction(stepOrAction: string[] | AnyRecord = {}) {
  const actions = Array.isArray(stepOrAction) ? stepOrAction : stepOrAction.action || [];
  return actions.includes("scroll");
}

export function normalizeActionTokens(action: string | string[] = ["step", "tooltip"]) {
  const values = Array.isArray(action) ? action : [action];
  return uniqueActionTokens(values.flatMap((value) => {
    if (value === "stepper") return ["step", "tooltip"];
    if (value === "scroller") return ["scroll", "tooltip"];
    return [value];
  }));
}

export function normalizeActionEvent(event: any, options: AnyRecord = {}, context: AnyRecord = {}) {
...
export function normalizeScrollAction(scrollSpec: true | AnyRecord = {}) {
