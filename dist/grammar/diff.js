...
export function diffViewStates(previous, next) {
    ...
    return {
        ...
        hasDelta: (type, action = null) => semantic.deltas.some((item) => item.type === type && (action == null || item.action === action)),
        semantic,
        previous: prev,
        next: curr
    };
...
export function diffSemanticViewStates(previous = {}, next = {}) {
    ...
    return {
        ...
        next: curr,
        deltas,
        has: (type, action = null) => deltas.some((item) => item.type === type && (action == null || item.action === action)),
