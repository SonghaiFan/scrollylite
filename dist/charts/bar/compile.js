...
function compileBarCoordinate(spec, guideSpec = {}, context = {}) {
    ...
    const category = channelFromField(categoryChannel(encoding), categoryChannel(encoding)?.title, "nominal");
    const measure = channelFromField(measureChannel(encoding), measureChannel(encoding)?.title, "quantitative");
...
function semanticKeyFromEncoding(encoding = {}, previousSemanticKey = null) {
    ...
    return semanticKeyFromParts(previousSemanticKey?.entity || previousSemanticKey?.entities || { field: category.field }, { value: measure.field });
}
