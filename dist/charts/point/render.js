...
import { defaultPointRadius, parentAnchors, pointState, radiusScale } from './state.js';
...
class PointChart extends BaseChart {
    render(chart, rows, spec, tooltip, d3) {
        const { bandOrLinear, bindTooltip, colorScale, drawGrid, drawLegend, drawXAxis, drawYAxis, fadeNonPointShapes, niceExtent, position, quantitativeDomain, quantitativeScale, staggerDelay } = this.deps;
        ...
        if (state.granularityMode === 'aggregate' && state.parentField) {
            ...
            chart.g.selectAll('circle.sl-point-parent')
                ...
                .transition(t)
                .attr('r', 4)
                .style('opacity', 0.4), (update) => update.transition(t)
                .attr('cx', ([, p]) => p.x)
                .attr('cy', ([, p]) => p.y)
                .attr('r', 4)
                .style('opacity', 0.4), (exit) => exit.transition(t).attr('r', 0).style('opacity', 0).remove());
        }
        else {
            chart.g.selectAll('circle.sl-point-parent')
                .transition(t)
