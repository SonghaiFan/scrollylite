...
import { parentAnchors, parentKey, defaultPointRadius, radiusScale, pointState } from "./state.js";
...
class PointMarkRenderer extends BaseChart {
    render(chart, rows, spec, tooltip, d3) {
        const { bandOrLinear, bindTooltip, colorScale, drawLegend, fadeNonPointShapes, position, quantitativeDomain, quantitativeScale, staggerDelay } = this.deps;
        ...
        const key = pointKeyAccessor(spec, enc.x?.field);
        ...
        chart.g
            ...
            .attr("r", (d) => radius(d)), (update) => update
            .call(applyPointIdentity, key)
            .call(bindTooltip, spec, tooltip)
            .transition(t)
            .delay((d, i) => staggerDelay(spec, d, i))
