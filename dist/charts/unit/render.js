...
class UnitChart extends BaseChart {
    render(chart, rows, spec, tooltip, d3) {
        const { colorScale, drawLegend, escapeHtml, fadeNonUnitShapes, hideTooltip, moveTooltip, showTooltip, staggerDelay } = this.deps;
        ...
        chart.g
            ...
            .data(units, unitKey)
            .join((enter) => enterUnit(enter), (update) => updateUnit(update), (exit) => exitUnit(exit));
        chart.scene.unitPositions = new Map(units.map((datum, index) => [
            unitKey(datum),
            {
        ...
        function exitUnit(exit) {
            const transition = exit
                ...
                .attr("r", 0);
            if (hasXAxis)
                transition.attr("cy", chart.innerHeight + layout.r);
        ...
        function bindUnitTooltip(selection) {
            ...
            selection
                ...
                const bits = [];
                if (labelKey)
                    bits.push(`${escapeHtml(labelKey)}: ${escapeHtml(d.__row[labelKey])}`);
                if (valueKey)
                    bits.push(`${escapeHtml(valueKey)}: ${escapeHtml(d.__row[valueKey])}`);
...
function unitMotionTiming(spec, chart, d3, hasXAxis) {
    ...
    const duration = motion.duration || Math.round(baseDuration * (hasXAxis ? DEFAULT_TIMING.unit.axisDurationMultiplier : 1));
    ...
    const xStagger = mergeStagger(DEFAULT_TIMING.unit.xStagger, motion.xStagger ?? unit.xStagger ?? motion.gravityStagger ?? unit.gravityStagger);
...
function mergeStagger(defaults, override) {
    ...
    if (typeof override === "number")
        return { step: override, max: defaults.max };
